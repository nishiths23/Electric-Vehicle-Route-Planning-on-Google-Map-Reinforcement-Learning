...
env = Environment('18.526767, 73.927861', '18.511426, 73.878852') # Delhi - '28.656534,77.229898')  # ('40.468254,-86.980963', '40.445283,-86.948429')
...
parameter = [env.map_bound, map_height, map_wide, step_length, wide_grid_num, height_grid_num, total_point, max_train_step, pre_train_step, learning_rate, step_rewardG, chargenumG,
             SOCG, timeG]
df_1 = pd.DataFrame([parameter])
df_1.to_csv("./ev/train_para.csv",
            header=["google map_boundary", "map_height(km)", "map_wide(km)", "step_length(m)", "wide_grid_num", "height_grid_num", "total_point", "max_train_step in episode",
...
with tf.Session() as sess:
    ...
    for episode in range(train_num):  # num of episode
        ...
        while (in_ep_step <= max_step):  # Max step in one episode
            ...
            print('current step - ' + str(in_ep_step) + ' max steps - ' + str(max_step) + ' episode - ' + str(episode_num) + ' total episodes - ' + str(train_num), end = '\r')
            ...
            if (total_step > pre_train and env.status_dir_check != 'OVER_QUERY_LIMIT' and len(replay_buffer.buffer) > batch_num) or (
            ...
            if d == True:
                ...
                time = env.time
                history = [episode + 1, in_ep_step, time, episode_reward, real_reward, real_r_nofail, charge_num, SOC, env.unreach_position_num, d,
                           random_a / (random_a + network_a), avg_loss, overQ_num, loss_history, step_buffer]
                if episode == 0:
                    df = pd.DataFrame([history])
                    df.to_csv("./ev/result.csv",
                              header=["episode", "step", "time", "reward", "reward_notrain", "reward_nofail", "charge_num", "SOC", "unreach_position", "Reach", "Random_a",
                                      "Avg_loss", "overQuery_num", "Loss history", "Step history"])
                    tt = tt + 1
                elif episode > 0:
